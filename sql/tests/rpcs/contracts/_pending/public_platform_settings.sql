...
-- ────────── 5. returns only public fields (not backup_cron_expr or updated_by) ──────────
SELECT ok(
  (NOT (rpc_public_platform_settings()::jsonb ? 'backup_cron_expr')
   AND NOT (rpc_public_platform_settings()::jsonb ? 'updated_by')),
  'response does NOT expose backup_cron_expr or updated_by'
);
