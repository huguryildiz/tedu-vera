...
│   └── 007_security_policy_enforcement.sql ← Security policy JSONB schema + patched RPCs
...
| 007 | `007_security_policy_enforcement.sql` | Patch: update `security_policy` JSONB default, rename `ccSuperAdminOnPinReset` → `ccOnPinReset`, add `ccOnScoreEdit`; re-patch `rpc_jury_verify_pin` and `rpc_admin_generate_entry_token` to read policy at runtime |
...
| Table | Key columns |
|-------|-------------|
| `organizations` | `code` UNIQUE, `name`, `subtitle`, `status`, `settings JSONB` |
...
| `rpc_admin_generate_entry_token(period_id)` | Create entry token; reads `tokenTtl` from `security_policy`; plain value returned once, hash stored |
