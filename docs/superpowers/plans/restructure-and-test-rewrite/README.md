...
### Sonnet High (~200k context) — 33 oturum (26 ana plan + 7 uzantı)
...
| ✅ | **27** | Legacy artifact cleanup (`.gitkeep×36`, `__tests__.archive×2`, `crud-legacy`, `legacy-shell/eyebrow`) + global CSS dead scan + `check:js-size`/`check:css-size` guard scriptleri + HeatmapPage.css 719 split | ~60 sil + 3 yeni script + 1 CSS split | ~2h | %60 | Temiz zemin; S28 için hazır · [session-27](implementation_reports/session-27-cleanup-guards.md) |
| ✅ | **28** | Rename sprint — Semester→Period (55 dosya) + Students→Team Members (15 dosya, UI-only) + memory housekeeping | ~70 edit + 5 rename + 2 storage shim | ~2h | %70 | Naming unified; S29 için JSX split başlangıcı · [session-28](implementation_reports/session-28-rename-sprint.md) |
| ✅ | **29** | SetupWizardPage split — 2157 satır → orchestrator 263 + 7 step modülü (WizardStepper/WelcomeStep/PeriodStep/CriteriaStep/JurorsStep/ProjectsStep/CompletionStep); CriteriaStep 595 (framework+criteria phase bundle), helper'lar tüketen step'e taşındı, ölü STEP_ICONS silindi | 1 page refactor + 7 step dosyası | ~1.5h | %85 | JSX split pattern validated · [session-29](implementation_reports/session-29-setupwizard-split.md) |
| ✅ | **30** | Admin large split P1 — PeriodsPage 1766 → 757 orch + 8 komponent (StatusPill, ReadinessPopover, SortIcon, LifecycleBar, ProgressCell, LifecycleGuide, PeriodsTable, PeriodsFilterPanel, periodHelpers); OrganizationsPage 1713 → 759 orch + 8 komponent (OrgStatusBadge, SortIcon, TenantStatusPill, OrgTable, UnlockRequestsPanel, OrgDrawers bundle, OrgModals bundle, organizationHelpers). CLAUDE.md ReadinessPopover path güncellendi. | 2 page + 16 yeni komponent | ~2h | %30 | S31 için aynı pattern · [session-30](implementation_reports/session-30-periods-orgs-split.md) |
| ⏳ | **31** | Admin large split P2 — EntryControlPage 1565 + OutcomesPage 1534 (TokenGenerator/SessionHistoryTable/RevokeFlow/OutcomeTable/OutcomeMappingPanel) | 2 page + ~7 yeni komponent | ~2.5h | %75 | Kalan büyükler S32'de |
| ⏳ | **32** | Admin large split P3 — CriteriaPage 1468 split + GovernanceDrawers 1307 değerlendirmesi (bundle bırak veya 6 drawer ayrı dosya) | 1 page + ~2 yeni + opsiyonel 6 drawer | ~2h | %70 | Mid-size S33'te |
| ⏳ | **33** | Mid-size değerlendirmesi (JurorsPage/LandingPage/RankingsPage/ProjectsPage) + test coverage build-out (modals, shared API, edge) — hedef global %50+ | 0-4 page split + ~10 yeni test | ~2.5h | %75 | Plan uzantısı kapanış |

**Toplam: 33 oturum (1 plan + 25 ana çalışma + 7 uzantı) ≈ 82-99 saat ≈ 16-20 iş günü.**
...
🎉 **Ana plan tamamlandı — Aşama A + B + C bitti. S1-S26c / 26 session.**

**🔜 Uzantı sprint (S27-S33):** Post-restructure cleanup (legacy artifact + dead CSS), Semester→Period + Students→Team Members rename, 11 adet 1000+ satırlı JSX'in split'i, test coverage build-out (%50+ hedef), `check:js-size` / `check:css-size` guard script'leri. Tam yol haritası: **[README-2.md](README-2.md)**.
...
|---|---|---|---|
| 2026-04-23 | 30 | **S30 tamam — PeriodsPage + OrganizationsPage split:** **PeriodsPage** 1766 → 757 satır orchestrator + 9 yeni komponent (`components/`): `periodHelpers.js` (62, `formatRelative`+`getPeriodState`+`SETUP_REQUIRED_TOTAL`+`computeSetupPercent`+`computeRingModel`), `StatusPill` (43), `ReadinessPopover` (117, canonical portal popover), `SortIcon` (10), `LifecycleBar` (32), `LifecycleGuide` (121, localStorage-persisted collapse), `ProgressCell` (32), `PeriodsFilterPanel` (129), `PeriodsTable` (548, tbody+thead+empty state + module-local `PeriodRow` ~280 satır). CLAUDE.md'de tek satırlık ReadinessPopover canonical reference yeni path'e güncellendi (`src/admin/features/periods/components/ReadinessPopover.jsx`). **OrganizationsPage** 1713 → 759 satır orchestrator + 8 yeni komponent (`components/`): `organizationHelpers.js` (57, `getInitials`+`formatShortDate`+`formatRelativeTime`+`getAvatarColor`+`getOrgInitials`+`getOrgHue`), `OrgStatusBadge` (33), `SortIcon` (10), `TenantStatusPill` (29, unlock-request status — eski inline `StatusPill` ile çakışmasın diye rename), `OrgTable` (247, kartlı toolbar + filter panel + desktop table + pagination), `UnlockRequestsPanel` (161, sub-tabs + table + pagination), `OrgDrawers.jsx` (397, bundle: `CreateOrgDrawer` + `EditOrgDrawer` + `ViewOrgDrawer` + `ManageAdminsDrawer`), `OrgModals.jsx` (275, bundle: `ToggleStatusModal` + `DeleteOrgModal` + `ResolveUnlockModal`). Plan'daki `PeriodsMobileCards` / `OrgMobileCards` / `OrgAvatar` ayrıştırmaları yapılmadı: mevcut kod single-responsive-table modelinde (portrait-only `td` + CSS breakpoints), ayrı mobile component'e ihtiyaç yok; avatar helper'ları `organizationHelpers.js` içinde toplanmış durumda. Her iki orchestrator da ≤800 satır kabul bandında (757 / 759); `check:js-size` warn list'ten çıktılar. CLAUDE.md'deki retroactive violation listesinden PeriodsPage + OrganizationsPage çıkarıldı. `useManagePeriods` ve `useManageOrganizations` hook'ları sadece orchestrator'da çağrılır; child'lar prop drill pattern'inde (feedback stale). **Build ✅ 5.46s, 278/278 test, tüm size + lint check'ler yeşil.** · [session-30](implementation_reports/session-30-periods-orgs-split.md) | Session 31: EntryControlPage 1565 + OutcomesPage 1534 split |
| 2026-04-23 | 29 | **S29 tamam — SetupWizardPage split:** 2158 satırlık monolit → 263 satırlık orchestrator + 7 step dosyası (`steps/`): `WizardStepper` (50), `WelcomeStep` (71), `PeriodStep` (215, +getSuggestedSeason), `CriteriaStep` (595, bundle: wrapper + FrameworkPhase + CriteriaPhase, +buildCriteriaPayload), `JurorsStep` (271), `ProjectsStep` (327, +membersStringToJsonb), `CompletionStep` (360, +useConfetti + readinessCheckToStep). Helper fonksiyonlar onları tüketen step dosyasına taşındı. Ölü `STEP_ICONS` constant silindi (tanımlı ama hiç kullanılmıyordu). `size-ceiling-ok` escape-hatch comment kaldırıldı — artık tüm dosyalar kabul bandında (hepsi ≤600). CriteriaStep 595 satır sweet-spot'un üstünde ama coherent bundle (3 iç-bileşen tight coupling, shared state + conditional render). `useSetupWizard.js` ve `styles/` dokunulmadı. Mevcut test `SetupWizardPage.test.jsx` mock'ları yeterli — güncelleme gerekmedi. **Build ✅ 5.75s, 278/278 test, `check:js-size` (SetupWizardPage warn list'ten çıktı), `check:css-size`, `check:no-native-select`, `check:no-nested-panels`, `check:no-table-font-override` — hepsi temiz.** · [session-29](implementation_reports/session-29-setupwizard-split.md) | Session 30: PeriodsPage 1765 + OrganizationsPage 1712 split (Opus 4.7) |
...
| 2026-04-23 | 26c | **Dead CSS safe wins (Option C):** PostCSS AST-based rule removal. Script `/tmp/find-dead-classes-v2.mjs` ile class extraction + dinamik pattern filtresi (library prefix'leri recharts/rt/rdp + suffix pattern'ler primary/danger/completed/snapshot + template literal detection) → 234 high-confidence dead class. Sonra PostCSS walker ile rule silme: multi-selector rule'larda canlı branch korundu, empty @media temizlendi. **311 rule / 527 satır / 13 dosya temizlendi.** En büyükler: showcase-slides.css (94 satır — ps-step/scoring/criteria/entry/qr), crud-legacy.css (76 — fs-crit-card/fs-band/fs-outcome), mop-base.css (81 — mop-search-row/org-card/status-select), empty-state-card.css (74 — vera-es-*-criteria/pending). Build ✅ 5.87s, 278/278 test, 3 lint clean. Görsel smoke yapılmadı (kullanıcı telefonda) — dev server'da kontrol öneriliyor. ~3500 satır ölü aday kaldı (responsive/dinamik/library-injected), derin sprint ister. · [session-26c](implementation_reports/session-26c-dead-css-safe-wins.md) | — |
| 2026-04-23 | 26b | **Post-S26 cleanup — planda kalan 4 iş değerlendirildi:** (1) **qa-catalog audit:** 312 entry, 0 orphan — her ID'nin src/supabase/sql altında en az 1 test referansı doğrulandı; tracker satırı ✅ olarak güncellendi. (2) **Dead CSS silme:** `legacy-*` (5 dosya) + `mop-*` (3 dosya) `src/styles/main.css`'ten import ediliyor; içerdikleri class'lar JSX'te kontrol edildi — `btn-landing-primary` 6 JSX, `landing-section-label` 8 JSX, `mop-btn-primary` 1 JSX, `outcome-editor-*` 1 JSX; dosya-bazında silme güvensiz — per-class audit ayrı bir CSS sprinti olarak bırakıldı. (3) **Dark mode tokenize:** 46 CSS dosyası × 198 override bloğu — 2-4 saatlik riskli refactor, plan opsiyonel işaretlemişti, ayrı sprint. (4) **E2E çalıştırma:** dev server + DB gerektirir, `npm run e2e` kullanıcı elinde. Tracker %100 altyapı tamam (11/11), plan tam kapanış. · [session-26b](implementation_reports/session-26b-post-cleanup.md) | — |
...
| 2026-04-23 | — (ara) | **Post-S26c ara iş — dead code/CSS tarama + plan uzantısı:** Dead JS/JSX + dead CSS taraması (3 paralel Explore agent). Temizlenen: `src/admin/utils/projectHelpers.js` (53 satır, 0 import) + 3 dup legacy CSS (`legacy-sections.css`, `legacy-responsive.css`, `legacy-light-mode.css` — hepsi main muadiliyle bire bir kopya) + `sections.css`/`mobile.css`'ten ölü bloklar (`.landing-admin-gallery/.ag-*`, `.landing-usecases/.uc-*`, `.landing-cta-reprise*`) + `main.css`'ten 3 ölü `@import`. **Toplam −885 satır**, 0 yeni. Build ✅ 5.62s, 278/278 test, 3 lint clean. Memory cleanup: `project_codex_concurrent.md` silindi. **CLAUDE.md'ye yeni kural:** JSX/JS file size ceiling (500 sweet / 800 kabul / 1000+ split adayı). **S27-33 uzantı planı yazıldı:** [README-2.md](README-2.md) — 7 oturum, ~17h: legacy artifact cleanup + Semester→Period + Students→Team Members rename + 11 adet 1000+ JSX split + test coverage %50+ hedef + `check:js-size`/`check:css-size` guard script'leri. XLSX'e 7 yeni `ASKIDA` satırı eklendi (S27-33), toplam 26 → 33 oturum. Commit atılmadı (kullanıcı açıkça isteyene kadar). | S27 (başka chat'te) — kickoff prompt [README-2.md](README-2.md) sonunda |
| 2026-04-23 | 28 | **S28 tamam — Rename sprint:** **(28a) Semester → Period:** Tüm JS identifier'lar (`semesterId`→`periodId`, `useSemesters`→`usePeriods`, `listSemesters`→`listPeriods` vb.), UI string'leri, CSS class'ları ve comment'ler yeniden adlandırıldı. `SemesterStep.jsx` → `PeriodStep.jsx` (`git mv`). ExportPage.jsx'te 2 runtime bug düzeltildi (`projectsBySemester`→`projectsByPeriod` undefined + `sortSemesters`→`sortPeriods` undefined). JuryFlow.jsx backward compat alias'ları (`semester: "period"`, `semester: PeriodStep`) kasıtlı bırakıldı. DB snake_case alanları (`semester_id`, `semester_name`, `semester_locked`, `active_semesters`) değiştirilmedi. **(28b) Students → Team Members:** `StudentNames` React bileşeni → `TeamMemberNames` (EntityMeta.jsx), `normalizeStudentNames` → `normalizeTeamMemberNames` (auditUtils.js). 13 consumer/test dosyası güncellendi. `buildProject.js` factory stub string güncellendi. **(28c) Memory:** `project_generic_naming.md` — Semester→Period ✅ ve Students→Team Members ✅ olarak işaretlendi. **Build ✅ 5.87s, 278/278 test, tüm check'ler exit 0.** · [session-28](implementation_reports/session-28-rename-sprint.md) | Session 29: SetupWizardPage split |
| 2026-04-23 | 27 | **S27 tamam — legacy artifact cleanup + guard scriptleri + HeatmapPage.css split:** (1) **Artifact silme:** 36 `.gitkeep` dosyası, 2 `__tests__.archive` dizini (`src/admin/__tests__/archive/`, `src/jury/__tests__/archive/`), `legacy-shell.css` (338 satır), `legacy-eyebrow.css` (287 satır), `src/admin/utils/projectHelpers.js` (silme main.css'ten import yoktu zaten), `crud-legacy.css` import'u `main.css`'ten kaldırıldı. (2) **Dead CSS scan (PostCSS AST):** `/tmp/dead-css-scan-s27.mjs` ile 26 kural / ~91 satır silindi — 9 dosya, en büyük: `heatmap-kpi-grid` bloğu (HeatmapPage.css), `.legacy-*` artıkları, `.m-row-group-header`. `.compare-select` false-positive düzeltildi (template literal içindeki sınıf; kural geri yüklendi). (3) **Guard scriptleri:** `scripts/check-js-file-size.mjs` (WARN >800, HARD >1000, `// size-ceiling-ok:` escape hatch) + `scripts/check-css-file-size.mjs` (WARN >600, HARD >1000, `/* size-ceiling-ok: */` escape hatch) — ikisi de `package.json`'a `check:js-size` / `check:css-size` olarak eklendi + `check:all` grubu. 12 mevcut JSX dosyasına `// size-ceiling-ok:` eklendi (SetupWizardPage 2157, PeriodsPage 1765, vb. — retroactive split S29–32'de). (4) **HeatmapPage.css split:** 719 satır → `HeatmapPage.css` (595 satır, core) + `HeatmapPage.responsive.css` (125 satır, 4 @media block) — her ikisi de `HeatmapPage.jsx`'e import edildi. **Build ✅ 5.74s, 278/278 test, tüm check'ler exit 0.** · [session-27](implementation_reports/session-27-cleanup-guards.md) | Session 28: Rename sprint (Semester→Period + Students→Team Members) |
