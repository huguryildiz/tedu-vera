...
| Sayfa | Unit | E2E | ✅ Bakılan | ❌ Bakılmayan |
|---|---|---|---|---|
| **Overview** | (1) `OverviewPage.test` | ❌ | Render (mock'lı) | KPI hesabı doğrulama, period switching, real-time update |
...
| **Jurors** ⭐ | (10) JurorsPage + table + 3 drawers/modals + helpers + lockEnforcement + responsiveTableMode | ✅ `jurors-crud.spec.ts` + `juror-batch-import.spec.ts` | ✅ CRUD + batch import | PIN regenerate → DB write doğrulama, juror→project assignment |
...
| Sayfa | Unit | E2E | ✅ Bakılan | ❌ Bakılmayan |
|---|---|---|---|---|
| **Analytics** | (3) page + useAnalyticsData + getOutcomeAttainmentTrends | ⚠️ `analytics.spec.ts` (5 test) + ✅ `outcome-attainment.spec.ts` | ✅ Attainment card text match, summary strip count, deep math (outcome-attainment) | **Chart numerical content, PDF export, trend chart period selection, all 10 chart sections data correctness** |
| **Reviews** | (2) page + useReviewsFilters | ❌ `reviews.spec.ts` (2 test, render only) | Render | Feedback gönderme, juror filter, project filter, score-edit-request inceleme |
| **Audit Log** | (2) page + filters | ✅ `audit-log.spec.ts` (10 test) | ✅ Filter UI, ✅ create/delete/login/token/project insert hepsi DB'de doğrulanıyor | Audit chain integrity verification (hash chain), pagination, export |
...
| **Settings** | (5) page + useAdminTeam + LastActivity + ChangePass + useProfileEdit | ❌ `settings.spec.ts` (2 test, render only) | Drawer açılıyor | **Tüm save akışları:** organization config, security policy, team member CRUD, change password gerçek persist |
...
| Skor | Sayfalar |
|---|---|
| **9/10 (sağlam)** | Jury Eval, Periods, Audit Log, Heatmap, Rankings (export math), Tenant Isolation (security) |
| **7/10 (kısmen)** | Login, ForgotPassword, ResetPassword, Organizations, Jurors CRUD, Projects CRUD, Setup Wizard submit, Pin Blocking, Entry Control, Outcome Attainment math |
| **5/10 (yüzeysel)** | Analytics (chart content), Setup Wizard (step persistence), Criteria (mapping), Outcomes (mapping), Tenant Application |
| **3/10 (çok zayıf)** | Reviews, Settings, Overview, CompleteProfile (OAuth), VerifyEmail, Export (full ZIP), Landing |

### Konsolide eksikler

**1. "Drawer açılıyor"da kalan ekranlar** (en kritik):

- **Settings** — security policy, team CRUD, change password save → DB persist hiç test edilmiyor
- **Reviews** — feedback gönderme, filtreleme yok
...
- Analytics 10 chart section'ından sadece 1-2'sinin sayısal doğrulaması var (attainment card)
- Heatmap haricinde chart-numerical-content assert yok
...
- Score-edit-request lifecycle (backend henüz yok)
...
İdeal seviyeye en hızlı yaklaştıran iş — toplam **~8 gün E2E iş**, infra değişimi gerektirmez:

1. **Settings save → DB persist E2E** (1-2 gün)
   Şu an tamamen kör; security policy değişikliği prod'da büyük etki

2. **Analytics XLSX hücre içerik testi** (2 gün)
   `outcome-attainment.spec.ts` patternini analytics export'a uygula, gerçek dataset → assert hücre değerleri

3. **Criteria mapping persist E2E** (2 gün)
   Outcome ↔ criterion mapping save → cascade attainment doğrulama

4. **Reviews feedback gönderme E2E** (1 gün)
   En az kapsamlı sayfa

5. **Setup wizard step-step persistence** (2 gün)
   Her adım gerçekten DB'ye yazıyor mu
